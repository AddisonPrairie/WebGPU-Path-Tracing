//from: http://www.graphics.cornell.edu/online/formats/rgbe/
#ifndef _H_RGBE
#define _H_RGBE
...
int hdrToFloats(int bufferSize, char* HDRIImageBuffer) {
    FILE* f = fopen("hello.bin", "wb");
    fwrite(HDRIImageBuffer, bufferSize, 1, f);
    fclose(f);
    FILE* f2 = fopen("hello.bin", "rb");

    int image_width, image_height;
    RGBE_ReadHeader(f2, &image_width, &image_height, NULL);
    float* image = (float*) malloc(sizeof(float) * 3 * image_width * image_height);
    RGBE_ReadPixels_RLE(f2, image, image_width, image_height);

    float* imagePadded = (float*) malloc(sizeof(float) * 4 * image_width * image_height);
    int size = image_width * image_height;
    for (int i = 0; i < size; i += 1) {
        imagePadded[i * 4 + 0] = image[i * 3 + 0];
        imagePadded[i * 4 + 1] = image[i * 3 + 1];
        imagePadded[i * 4 + 2] = image[i * 3 + 2];
        imagePadded[i * 4 + 3] = 3.1415;
    }

    int* outWrite = (int*) HDRIImageBuffer;
    outWrite[0] = image_width;
    outWrite[1] = image_height;
    outWrite[2] = (int) imagePadded;
    return 0;
}
}
...
int RGBE_ReadPixels(FILE *fp, float *data, int numpixels)
{
  ...
  while(numpixels-- > 0) {
    if (fread(rgbe, sizeof(rgbe), 1, fp) < 1) {
        printf("with %i pixels remaining\n", numpixels);
        return rgbe_error(rgbe_read_error,NULL);
    }
